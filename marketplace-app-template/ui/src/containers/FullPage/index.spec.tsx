// eslint-disable-next-line
import { render } from "@testing-library/react/pure";
// import FullPage from "./index";

test("Dummy test case", () => {
  expect(true).toBeTruthy();
});

// ID attribute of all the elements in config screen.
// const fullPageUIElementsIDs = [
//   '<idOfElementInFullPageScreen>'
// ];

// let fullPageRenderedDOM: any = null;
// beforeAll(async () => {
//   fullPageRenderedDOM = render(<FullPage />);
// });

// describe(`UI Elements of Full Page Screen`, () => {
//   fullPageUIElementsIDs.forEach((id: String) => {
//     test(`Rendered ${id} element`, () => {
//       expect(
//         fullPageRenderedDOM?.
//           container?.
//           querySelector(`#${id}`)
//       ).toBeTruthy();
//     });
//   });
// });

// describe(`Functional test cases`, () => {
//   List out all the functional test cases if any
//   For firing various events on HTML DOM Elements,
//   use functions like:
//   const inputElement = fullPageRenderedDOM?.container?.querySelector('#idOfDOMElement');
//   fireEvent.click(inputElement);
//   fireEvent.keyPress(inputElement);
//   fireEvent.keyDown(inputElement);
//   fireEvent.keyUp(inputElement);
// });
