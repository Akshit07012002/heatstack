...
const FlowFunnelVisualization: React.FC<FlowFunnelProps> = ({
  data,
  flowSequence,
}) => {
    ...
    <div className="flow-funnel-container">
      ...
      <div className="flow-funnel-chart">
        {funnelData.map((value, index) => {
          ...
          const dropOff =
            index > 0 ? getDropOff(value, funnelData[index - 1]) : null;
...
const HeatmapIframe: React.FC<HeatmapIframeProps> = ({
  url,
  type,
  metrics,
}) => {
    ...
    const sendFormattedMetrics = () => {
      try {
        ...
        if (iframe?.contentWindow) {
          ...
          console.log("FullPage - Formatted metrics sent to iframe:", {
            type,
            metricsToSend,
...
const FullPage: React.FC = function () {
  ...
  const [lyticsSegments, setLyticsSegments] = useState<
    Array<{ label: string; value: string }>
  >([]);
  const [selectedLyticsSegment, setSelectedLyticsSegment] = useState<{
    label: string;
    value: string;
  } | null>(null);
  ...
  const [flows, setFlows] = useState<
    Array<{ sequence: string[]; isSaved: boolean; id?: string; name?: string }>
  >([{ sequence: [], isSaved: false }]);
  const [expandedFlowIndex, setExpandedFlowIndex] = useState<number | null>(
    null
  );
  const [flowVisualizationData, setFlowVisualizationData] = useState<
    Record<number, any>
  >({});
  const [lyticsUserIds, setLyticsUserIds] = useState<string[]>([]);
  const [isFlowsSectionExpanded, setIsFlowsSectionExpanded] =
    ...
    ContentstackAppSdk.init()
      .then(async (appSdk) => {
        ...
        const environmentsData =
          ...
          environmentsResponse?.value?.environments ||
          [];
        const websites =
          environmentsData?.flatMap(
            (env: any) => env.urls?.map((urlObj: any) => urlObj.url) || []
          ) || [];
        ...
        try {
          ...
          console.log(
            "FullPage - Transformed Lytics segments:",
            segmentOptions
          );
        } catch (error) {
    ...
    try {
      if (typeof Storage !== "undefined") {
        ...
        for (let i = 1; i <= 100; i++) {
          ...
          if (savedFlowData) {
            try {
              ...
              console.warn(
                `FullPage - Failed to parse flow from localStorage key "${key}":`,
                parseError
              );
            }
          }
        ...
        if (savedFlows.length > 0) {
          console.log(
            "FullPage - Loaded saved flows from localStorage:",
            savedFlows
          );
          setFlows(savedFlows);
        }
  ...
  const fetchHeatmapData = async (heatmapType: string) => {
    ...
    try {
      ...
      const lyticsIds =
        selectedLyticsSegment && lyticsUserIds.length > 0
          ? lyticsUserIds
          : undefined;
      const response = await services.getHeatmapMetrics(
      ...
      const aggregatedMetrics: Record<
        string,
        { view: number; click: number; hover: number }
      > = {};
          ...
          if (user?.points && typeof user?.points === "object") {
            Object.keys(user?.points)?.forEach((dataCslpKey: string) => {
              ...
              if (Array.isArray(pointData?.click)) {
                aggregatedMetrics[dataCslpKey].click += pointData.click.reduce(
      ...
      console.log(
        "FullPage - Formatted metrics for",
        eventType,
        ":",
        formattedMetrics
      );
      ...
      setHeatmapData({
        ...
        url:
          websiteOptions?.find(
            (option: any) => option.value === selectedWebsite?.value
          )?.label || "",
      });
    } catch (err: any) {
      ...
      if (
        ...
        setError(
          "Unable to connect to the server. Please ensure the backend server is running on localhost:8080"
        );
      } else {
  ...
  const handleLyticsSegmentChange = async (option: any) => {
    ...
    try {
      ...
      if (response?.userIds && Array.isArray(response.userIds)) {
        ...
        userIds = response
          .map(
            (user: any) => user?.id || user?.userId || user?._id || String(user)
          )
          .filter(Boolean);
  ...
  const deleteFlow = (flowIndex: number) => {
    ...
    if (flowToDelete.isSaved && flowToDelete.name) {
      try {
        ...
        console.warn(
          "FullPage - Failed to remove flow from localStorage:",
          error
        );
      }
    }
  ...
  const removeStepFromFlow = (flowIndex: number, stepIndex: number) => {
    ...
    updatedFlows[flowIndex] = {
      ...updatedFlows[flowIndex],
      sequence: updatedFlows[flowIndex].sequence.filter(
  ...
  const updateStepInFlow = (
    ...
    updatedFlows[flowIndex] = {
      ...updatedFlows[flowIndex],
      sequence: updatedFlows[flowIndex].sequence.map((step, idx) =>
  ...
  const getAvailableOptionsForStep = (flowIndex: number, stepIndex: number) => {
    ...
    const selectedValues = currentFlow.sequence.filter(
      (_, idx) => idx !== stepIndex && _
    );
  ...
  const handleSaveFlow = async (flowIndex: number) => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "FullPage - Failed to save to localStorage:",
          storageError
        );
        ...
        <div className="fullpage-content">
          <div className="fullpage-header">
            <div className="fullpage-header-row">
              <div className="fullpage-header-section">
                <FieldLabel htmlFor="heatmap-type" className="fullpage-field">
                  Heatmap Type
                </FieldLabel>
                    ...
                    {heatmapOptions?.map((option) => (
                      <Button
                        ...
                        className={`heatmap-button ${
                          selectedHeatmap === option.value ? "selected" : ""
                        }`}
              ...
              <div className="fullpage-header-section">
                <FieldLabel htmlFor="website-select" className="fullpage-field">
                  Website
                </FieldLabel>
          ...
          <div
            className={`flows-section ${
              !heatmapData?.metrics?.aggregated ? "flows-disabled" : ""
            }`}
          >
            <div className="flows-header">
              <div className="flows-header-left">
                <Button
                  ...
                  iconOnly
                  onClick={() =>
                    setIsFlowsSectionExpanded(!isFlowsSectionExpanded)
                  }
            ...
            {isFlowsSectionExpanded && (
              <div className="flows-container">
                {flows.map((flow, flowIndex) => (
                  <div
                    key={flowIndex}
                    className={`flow-item ${flow.isSaved ? "flow-saved" : ""}`}
                  >
                    <div className="flow-header">
                      <FieldLabel
                        htmlFor={`flow-${flowIndex}`}
                        ...
                        <div key={stepIndex} className="flow-step">
                          ...
                          <div className="flow-step-input">
                            {getAvailableCslpOptions().length > 0 ? (
                              <Select
                                options={getAvailableOptionsForStep(
                                  flowIndex,
                                  stepIndex
                                )}
                                placeholder="Select data-cslp"
                                value={
                                  step ? { label: step, value: step } : null
                                }
                                onChange={(option: any) =>
                                  updateStepInFlow(
                                    flowIndex,
                            ...
                            {flow.sequence.length > 1 && !flow.isSaved && (
                              <Button
                                ...
                                iconOnly
                                onClick={() =>
                                  removeStepFromFlow(flowIndex, stepIndex)
                                }
                          ...
                          <div className="flow-visualization-content">
                            {isLoading ? (
                              ...
                              <FlowFunnelVisualization
                                ...
                                flowSequence={flow.sequence.filter(
                                  (step: string) => step
                                )}
                              />
                            ) : flowVisualizationData[flowIndex] ? (
